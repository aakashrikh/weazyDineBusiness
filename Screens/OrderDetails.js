    ...
    return (
      <View style={styles.container}>
        ...
        <ScrollView>
          ...
          <View style={{ flexDirection: "row", paddingHorizontal: 10, width: "100%", padding: 5, justifyContent: "space-evenly" }}>
            <View style={{ width: "50%" }}>
  ...
  productCard = ({ item }) => (
    <View style={style.card}>
      ...
      <View style={{ flexDirection: "row", width: "100%" }}>
        {/* View for Image */}
        <View style={{ width: "20%" }}>
          <Image source={{ uri: item.product.product_img }}
            style={style.logo} />
        </View>
        {/* View for Content */}
            ...
            <View style={{ width: 200, }}>
              ...
              <View style={{ marginTop: 20, flexDirection: "row" }}>
                <View style={{ backgroundColor: "#E6EFF6", width: 25, justifyContent: 'center', borderRadius: 3, borderWidth: 1, borderColor: "#186AB1", }}>
                  <Text style={{ alignSelf: "center" }}>{item.product_quantity}</Text>
                </View>

                <Icon name="close-outline" type="ionicon" size={20} style={{ left: 2 }} />

                <Text style={{ fontSize: RFValue(12, 580) }}> ₹ {item.product_price / item.product_quantity}</Text>
              </View>
            </View>
      ...
      <View>
        ...
        <View style={{ backgroundColor: "#fff", marginTop: 10, paddingBottom: 10 }}>
          <View style={[style.detailsView, { borderBottomWidth: 0, paddingVertical: 10, alignItems: "center" }]}>
            ...
            <Text style={[styles.h4, { fontSize: RFValue(12, 580) }]}>₹ {this.props.data.order_amount}</Text>
          </View>
          {this.props.data.cgst > 0 || this.props.data.sgst > 0 ?
            <View style={[style.detailsView, { paddingVertical: 0, paddingBottom: 10 }]}>
              <Text style={[styles.h4, { fontSize: RFValue(11.5, 580) }]}>Taxes & Other Charges</Text>
              <Text style={[styles.h4, { fontSize: RFValue(11.5, 580), color: "#ff9933" }]}>+ {this.props.data.cgst + this.props.data.sgst}</Text>
            </View>
            :
            <></>}
          ...
          {this.props.data.order_discount > 0 ?
            <View style={[style.detailsView, { paddingVertical: 0, paddingBottom: 10 }]}>
              <Text style={[styles.h4, { fontSize: RFValue(11.5, 580) }]}>Discount</Text>
              <Text style={[styles.h4, { fontSize: RFValue(11.5, 580), color: "green" }]}>- {this.props.data.order_discount}</Text>
            </View>
            :
            <></>}
          <View style={[style.detailsView, { borderBottomWidth: 0 }]}>
            <Text style={[styles.h4, { color: "#000", fontSize: RFValue(12, 580) }]}>Grand Total</Text>
            <Text style={{ fontSize: RFValue(12, 580), color: "#000" }}> ₹ {this.props.data.total_amount}/-</Text>
          </View>
        </View>
        ...
        <View style={{ backgroundColor: "#fff", marginTop: 5, paddingBottom: 10 }}>
          <Text style={{ fontSize: RFValue(11.5, 580), color: "#696969", fontWeight: "600", marginLeft: 10 }}>Customer Details</Text>

          <View style={{ padding: 5, paddingLeft: 12 }}>
            <Text style={[styles.h4, { color: "#000", fontSize: RFValue(12, 580) }]}>{this.props.user.name}</Text>
            <Text style={{ color: "#000", fontSize: RFValue(12, 580) }}>+91-{this.props.user.contact}</Text>
          </View>
          ...
          {this.props.data.order_type == "Delivery" ?
            <>
              <View style={{ padding: 5, paddingLeft: 12 }}>
                ...
                <Text style={{ color: "#000", fontSize: RFValue(12, 580) }}>{this.props.user.address}</Text>
              </View>
        ...
        <View style={{ marginBottom: 30 }}>
          {this.props.data.order_status == "placed" ? (
            <>
              {
                this.state.mark_complete_buttonLoading ?
                  ...
                  <View style={{ justifyContent: "space-evenly", flexDirection: "row", marginTop: 40 }}>
                    <TouchableOpacity style={style.acceptButton} onPress={() => this.props.change_order_status("confirmed")}>
                      <Text style={style.buttonText}>Accept</Text>
                    </TouchableOpacity>

                    <TouchableOpacity style={style.declineButton} onPress={() => this.props.change_order_status("cancelled")}>
            ...
            <>
              {
                this.state.mark_complete_buttonLoading ?
                  ...
                  :
                  <View style={{ justifyContent: "space-evenly", flexDirection: "row", marginTop: 40 }}>
                    <TouchableOpacity style={[style.acceptButton, { backgroundColor: "#EDA332", width: "50%" }]} onPress={() => this.props.change_order_status("processed")}>
                      <Text style={[style.buttonText]}>Order In Progress</Text>
                    </TouchableOpacity>

                    <TouchableOpacity style={[style.declineButton, { width: "40%" }]} onPress={() => this.props.change_order_status("cancelled")}>
                      <Text style={[style.buttonText, { color: "#000" }]}>Cancel Order</Text>
                    </TouchableOpacity>
                  </View>
              }
            </>
            ...
            this.props.data.order_type == "Delivery" ? (
              <>
                {
                  this.state.mark_complete_buttonLoading ?
                    ...
                    :
                    <View style={{ justifyContent: "space-evenly", flexDirection: "row", marginTop: 40 }}>
                      <TouchableOpacity style={style.acceptButton} onPress={() => this.props.change_order_status("out for delivery")}>
                        <Text style={style.buttonText}>Out for Delivery</Text>
                      </TouchableOpacity>

                      <TouchableOpacity style={style.declineButton} onPress={() => this.change_order_status("cancelled")}>
                        <Text style={[style.buttonText, { color: "#000" }]}>Cancel Order</Text>
                      </TouchableOpacity>
                    </View>
              ...
              <View style={{ justifyContent: "space-evenly", flexDirection: "row", marginTop: 40 }}>
                <TouchableOpacity style={style.acceptButton} onPress={() => this.props.change_order_status("completed")}>
